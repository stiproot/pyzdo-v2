...
docker run --name project-m-persist-worker-$1 \
    -it --detach \
    ...
    -e KAFKA_GROUP_ID=projectm-persist-worker-001 \
    -e KAFKA_AUTO_OFFSET_RESET=latest \
    -e WORKER_TOPIC=PROJECTM_CMD_PERSIST \
    ...
    -e AZURE_TENANT_ID= \
    img-project-m-persist-worker-$1
